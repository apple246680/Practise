    ...
    public class UnitTest1
    {
        ...
        public void CheckScore()
        {
            try
            {
                ...
                if (itemsScore.Count > 0)
                    throw new Exception($"No score associated with any of the properties or listings should have higher score than 5 and lower score than 0.{Environment.NewLine}Error Data: {String.Join(Environment.NewLine, itemsScore.Select(t => $"ItemScoreId : {t.ID}  Value : {t.Value}"))}");
            }
            catch (Exception ex)
            {
        ...
        public void CheckItemDate()
        {
            try
            {
                ...
                if (items.Count > 0)
                    throw new Exception($"No property or listing should have availabilities before 19/02/2017.{Environment.NewLine}Error Data : {String.Join(Environment.NewLine, items.Select(t => $"Item ID: {t.ID}"))}");
            }
            catch (Exception ex)
            {
                Assert.Fail(ex.Message);
        ...
        [TestMethod]
        [DataRow("dadada", "dadada", "dadadad")]
        [DataRow("aagag", "aagag", "aagag")]
        [DataRow("mahdi", "1234", "Min-Seo Young-Ho")]
        public void TestUserRegister(string username, string password, string fullname)
        {
            try
            {
                ...
                Assert.IsTrue(user.FamilyCount == 1, "Register FamilyCount value different.");
